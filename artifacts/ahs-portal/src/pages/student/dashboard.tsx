...
function StatCard({
  ...
  const inner = (
    <div className={`rounded-xl p-2.5 sm:p-3 border cursor-pointer transition-all hover:-translate-y-0.5 hover:shadow-md ${bg} ${border}`}>
      <div className="flex items-start justify-between gap-1.5">
        <div className="min-w-0 flex-1">
          <p className={`text-[8px] sm:text-[9px] font-bold uppercase tracking-widest mb-1 ${text} opacity-60`}>{label}</p>
          <p className={`text-sm sm:text-lg font-black leading-none truncate ${text}`}>{value}</p>
          <p className={`text-[9px] sm:text-[10px] mt-1 leading-tight ${text} opacity-55`}>{sub}</p>
        </div>
        <div className={`h-6 w-6 sm:h-7 sm:w-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${iconBg}`}>
          <Icon className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${text}`} />
        </div>
      </div>
    </div>
  );
